...
import NoticationModel from "../models/notificationModel";
...
export const getSingleCourse = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (isCacheExist) {
      ...
      await redis.set(courseId, JSON.stringify(course),'EX',604800) //7days expires
      res.status(200).json({
        success: true,
        course
      })
...
export const addReview = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (course) {
      ...
      course.ratings = avg / course.reviews.length;// eg: 5 rating + 4 rating = 9 rating ,9 rating / 2 rating = 4.5 course rating;
    }
